...
# from config import cache_dir,codes, dbloc, chunksize,chunkoverlap
...
#             document_embedding_cache=LocalFileStore(self.cache_dir)
#         )
...
#     def index_local_codebase(self, directory: str, language: str) -> None:
#         """Index the entire local codebase stored in `./codes`."""
#         for language in os.listdir(directory):
#             language_path = os.path.join(directory, language)
#             if os.path.isdir(language_path):
#                 print(f"Indexing {language} code from {language_path}...")
...
#                 # Store in vector DB
#                 try:
#                     self.client.get_or_create_collection(name=language)
#                 except Exception as e:
#                     print(f"Error creating collection for {language}: {e}")
...
#                 print(f"Indexed {len(chunks)} chunks for {language}")
...
#     def index_local_codebase(self, directory: str) -> None:
#         """Index the entire local codebase stored in `./codes`."""
#         for language in os.listdir(directory):
#             language_path = os.path.join(directory, language)
#             if os.path.isdir(language_path):
...
#         return splitter.split_documents(documents)
    ...
    #                     # existing_ids = set(doc.metadatas["full_path"] for doc in collection.get())
    #                     existing_data = collection.get(include=["metadatas"])
    #                     existing_ids = set(meta["full_path"] for meta in existing_data["metadatas"] if meta)  # Ensure meta exists
    ...
    #     time.sleep(30)  # Check every 30 seconds for new files
    ...
    #                 collection = self.client.get_or_create_collection(name=language)
    #                 existing_data = collection.get(include=["metadatas"])
    #                 existing_ids = set(meta["full_path"] for meta in existing_data["metadatas"] if meta)
    #             except Exception as e:
    #                 print(f"Error accessing collection for {language}: {e}")
    #                 existing_ids = set()
    ...
    #                 collection_name = language  # Assign the collection name
    #             else:
    #                 print(f"No new documents to add for {language}")

    #     return collection_name  # Ensure function returns the collection name
import os
import ollama
import time
from typing import List, Dict, Any, Optional
